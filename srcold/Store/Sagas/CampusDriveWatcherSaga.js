...
function* getCampusDriveAcceptedInvitesListRequest(action) {
  ...
  try {
    const response = yield call(
      getCampusDriveAcceptedInvitesList,
      action.payload.page,
      action.payload.size
    );
...
function* getOnCampusJobDetailsApplyToJobsHcAndStudentInfoRequestSaga(action) {
  try {
    const response = yield call(
      getOnCampusJobDetailsApplyToJobsHcAndStudentInfoRequest,
      action
    );
...
export default function* CampusDriveWatcherSaga() {
  yield takeLatest(
    ACTION_GET_CAMPUS_DRIVE_INVITES_REQUEST,
    getCampusDriveInvitesRequestSaga
  );
  yield takeLatest(
    ACTION_POST_RESPOND_TO_CAMPUS_DRIVE_REQUEST_REQUEST,
    postRespondToCampusDriveRequestRequest
  );
  yield takeLatest(
    ACTION_GET_CAMPUS_DRIVE_ACCEPTED_INVITES_LIST_REQUEST,
    getCampusDriveAcceptedInvitesListRequest
  );
  yield takeLatest(
    ACTION_POST_CAMPUS_DRIVE_CLOSE_REQUEST,
    postCampusDriveCloseRequestSaga
  );
  yield takeLatest(
    ACTION_GET_CAMPUS_DRIVE_STUDENT_RESUME_REQUEST,
    getCampusDriveStudentResumeSaga
  );
  yield takeLatest(
    ACTION_GET_STUDENT_GAPS_INFO_REQUEST,
    getStudentGapsInfoRequestSaga
  );
  yield takeLatest(
    ACTION_GET_STUDENT_PROFILE_AND_HIRING_REQUEST,
    getOnCampusJobDetailsApplyToJobsHcAndStudentInfoRequestSaga
  );
}
