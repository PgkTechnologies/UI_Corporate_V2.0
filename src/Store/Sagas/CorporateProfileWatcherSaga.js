...
import {
  ACTION_GET_CORPORATE_PROFILE_REQUEST,
  ACTION_PATCH_CORPORATE_PROFILE_REQUEST,
  ACTION_POST_PUBLISH_CORPORATE_PROFILE_REQUEST,
  ACTION_PAYMENT_CORPORATE_EMAIL_REQUEST,
  ACTION_POST_UNIVERSITY_INVOICE_REQUEST,
} from "../Actions/SagaActions/SagaActionTypes";
import { actionUpdateGlobalLoaderSagaAction } from "../Actions/SagaActions/CommonSagaActions";
import {
  actionGetCorporateProfileResponse,
  actionGetCorporateProfileSagaAction,
} from "../Actions/SagaActions/CorporateProfileSagaActions";
...
function* postPublishCorporateProfileRequestSaga(action) {
  ...
  try {
    ...
    formData.append(
      "publishData",
      JSON.stringify(action.payload.apiPayloadRequest)
    );
...
export default function* CorporateProfileWatcherSaga() {
  yield takeLatest(
    ACTION_GET_CORPORATE_PROFILE_REQUEST,
    getCorporateProfileRequestSaga
  );
  yield takeLatest(
    ACTION_PATCH_CORPORATE_PROFILE_REQUEST,
    patchCorporateProfileRequestSaga
  );
  yield takeLatest(
    ACTION_POST_PUBLISH_CORPORATE_PROFILE_REQUEST,
    postPublishCorporateProfileRequestSaga
  );
  yield takeLatest(
    ACTION_PAYMENT_CORPORATE_EMAIL_REQUEST,
    getPaymentEmailDetailsRequestSaga
  );
  yield takeLatest(
    ACTION_POST_UNIVERSITY_INVOICE_REQUEST,
