...
import {
  ACTION_GET_PUBLISH_HISTORY_REQUEST,
  ACTION_GET_PUBLISH_HISTORY_ITEM_BY_ID_REQUEST,
...
function* getPublishHistoryItemByIdRequestSaga(action) {
  ...
  try {
    const response = yield call(
      getPublishHistoryItemByIdRequest,
...
export default function* PublishHistoryWatcherSaga() {
  yield takeLatest(
    ACTION_GET_PUBLISH_HISTORY_REQUEST,
    getPublishHistoryRequestSaga
  );
  yield takeLatest(
    ACTION_GET_PUBLISH_HISTORY_ITEM_BY_ID_REQUEST,
    getPublishHistoryItemByIdRequestSaga
  );
}
