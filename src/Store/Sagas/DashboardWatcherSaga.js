...
import {
  ProposalStoreData,
  actionUpdateGlobalLoaderSagaAction,
} from "../Actions/SagaActions/CommonSagaActions";
...
function* getUniversityListRequestSaga(action) {
  ...
  try {
    const response = yield call(
      getUniversityRegisteredRequest,
      action.payload.page,
      action.payload.size
    );
...
export default function* DashboardWatcherSaga() {
  ...
  yield takeLatest(
    ACTION_GET_CORPORATE_PROFILE_STATS_REQUEST,
    getCorporateProfileStatsSaga
  );
  yield takeLatest(
    ACTION_GET_CORPORATE_LIST_REQUEST,
    getCorporateListRequestSaga
  );
  yield takeLatest(
    ACTION_GET_UNIVERSITY_LIST_REQUEST,
    getUniversityListRequestSaga
  );
  yield takeLatest(
    ACTION_GET_PROPOSAL_DATA_AVAILABLE_REQUEST,
    getProposalDataRequestSaga
  );
}
