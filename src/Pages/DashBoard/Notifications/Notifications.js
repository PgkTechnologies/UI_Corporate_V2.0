...
const Notifications = () => {
  ...
  useEffect(() => {
    if (selectedOpt === "U") {
      const USender = notifications?.filter(
        (sender) => sender.senderType === "U"
      );

      setFilteredNtf(USender);
    } else if (selectedOpt === "C") {
      const CSender = notifications?.filter(
        (sender) => sender.senderType === "C"
      );
  ...
  const handleSelectAll = () => {
    ...
    const filteredQ = myQ?.filter(
      (item) => !nftID?.find((n) => n.notificationId === item.notificationId)
    );
  ...
  const handleIndividualCheck = (notificationId) => {
    ...
    const checked = nftID.some(
      (item) => item.notificationId === notificationId
    );
    const updatedNftID = checked
      ? nftID.filter((item) => item.notificationId !== notificationId)
  ...
  return (
    <div className="container-body" style={{marginTop:'100px'}}>
      ...
      >
        <div>
          {" "}
          <h1 style={{ color: "white", margin: "20px 30px 20px" }}>
            Notifications
          </h1>{" "}
        </div>
        <div className="btnBlock" style={{ paddingLeft: "25px" }}>
          <div
            ...
            style={{ paddingRight: "10px", textAlign: "none" }}
          >
            <input
              type="checkbox"
                  ...
                  (item?.readNotifications?.filter(
                    ...
                    >
                      <input
                        ...
                        onClick={() =>
                          handleIndividualCheck(item?.notificationID, index)
                        }
                      />
                      <label
