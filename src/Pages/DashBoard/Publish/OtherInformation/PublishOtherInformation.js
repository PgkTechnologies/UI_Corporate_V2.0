...
const PublishOtherInformation = () => {
    ...
    switch (name) {
      ...
      case "attachment":
        if (value.target.files) {
          ...
          if (
            parseFloat((value.target.files[0].size / 1024).toFixed(2)) > 5000
          ) {
  ...
  const isFormValid = () => {
    ...
    if (isValid) {
      if (
        (otherInfo?.title?.isRequired && otherInfo?.title?.error) ||
        (otherInfo?.information?.isRequired && otherInfo?.information?.error)
      ) {
        isValid = false;
      } else {
        isValid = true;
    ...
    if (isValid) {
      if (
        otherInfo?.title?.isRequired &&
        otherInfo?.title?.value &&
        otherInfo?.title?.value?.trim() !== ""
      ) {
        isValid = true;
      } else {
        isValid = false;
    ...
    if (isValid) {
      if (
        otherInfo?.information?.isRequired &&
        otherInfo?.information?.value &&
        otherInfo?.information?.value?.trim() !== ""
      ) {
        isValid = true;
      } else {
        isValid = false;
  ...
  const addOtherInformation = () => {
    if (isFormValid()) {
      ...
      dispatch(
        actionPostAddOtherInformationRequest({
