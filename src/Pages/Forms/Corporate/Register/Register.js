...
const Register = () => {
  ...
  const [corporatePrimaryData, setCorporatePrimaryData] =
  ...
  useEffect(() => {
    ...
    const isLocalStorageAvailable =
      storeData && Object.keys(storeData).length > 5 ? true : false;
    setPath(base64Img);
  ...
  const validateRefCode = (model) => {
    dispatch(
      actionValidateReferralRequestSaga({
        apiPayloadRequest: model,
        callback: onValidationResponse,
      })
    );
  };
