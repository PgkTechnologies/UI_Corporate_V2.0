...
const CorporateSecondary = () => {
  ...
  const countryCodes = useSelector(
    (state) => state.CorporateReducer.countryCodes
  );
  ...
  useEffect(() => {
    ...
    const isLocalStorageAvailable =
      storeData && Object.keys(storeData).length > 9 ? true : false;
    ...
    if (response) {
      ...
      if (storeData.corporateHQAddressState) {
        getCitiesByStateName(
          storeData?.corporateHQAddressCountry,
          storeData?.corporateHQAddressState,
          "HQ"
        );
      }
      ...
      setTimeout(() => {
        ...
        if (storeData?.corporateLocalBranchAddressState) {
          getCitiesByStateName(
            storeData?.corporateLocalBranchAddressCountry,
            storeData?.corporateLocalBranchAddressState,
            "LOCAL"
          );
        }
      }, 1500);
  ...
  const handleChange = (event) => {
    ...
    if (
      ["corporateHQAddressPhone", "corporateLocalBranchAddressPhone"].includes(
        name
      )
    ) {
    ...
    if (name === "corporateHQAddressState") {
      getCitiesByStateName(
        corporateSecondary.corporateHQAddressCountry,
        value,
        "HQ"
      );
    } else if (name === "corporateLocalBranchAddressState") {
      getCitiesByStateName(
        corporateSecondary.corporateLocalBranchAddressCountry,
        value,
        "LOCAL"
      );
    }
  ...
  const setStatesByType = (data, type) => {
    let statesList =
      data && data.length >= 0
        ? data?.map((item, i) => ({ value: item?.name, label: item?.name }))
        : null;
    if (type === "HQ") {
  ...
  const setCitiesByType = (data, type) => {
    let citiesList =
      data && data.length >= 0
        ? data?.map((item, i) => ({ value: item, label: item }))
        : null;
    if (type === "HQ") {
  ...
  const saveData = (event) => {
    ...
    if (isCheked) {
      ...
      setCorporateSecondary((preState) => ({
        ...preState,
        corporateLocalBranchAddressLine1:
          corporateSecondary.corporateHQAddressLine1,
        corporateLocalBranchAddressLine2:
          corporateSecondary.corporateHQAddressLine2,
        corporateLocalBranchAddressCountry:
          corporateSecondary.corporateHQAddressCountry,
        corporateLocalBranchAddressState:
          corporateSecondary.corporateHQAddressState,
        corporateLocalBranchAddressCity:
          corporateSecondary.corporateHQAddressCity,
        corporateLocalBranchAddressDistrict:
          corporateSecondary.corporateHQAddressDistrict,
        corporateLocalBranchAddressZipCode:
          corporateSecondary.corporateHQAddressZipCode,
        corporateLocalBranchAddressPhone:
          corporateSecondary.corporateHQAddressPhone,
        corporateLocalBranchAddressEmail:
          corporateSecondary.corporateHQAddressEmail,
      }));
    } else {
      setCode2("");
      setStateListLocal([]);
    ...
    if (selectedName === "Corporate") {
      if (
        ...
        const countryCode = countries?.find(
          (item) =>
            item?.value === corporateSecondary?.corporateHQAddressCountry
        );
        const countryCode2 = countries?.find(
          (item) =>
            item?.value ===
            corporateSecondary?.corporateLocalBranchAddressCountry
        );
              ...
              <div className="registration-card">
                <div style={{ width: "100%" }}>
                  <div className="register-as-text">
                    ...
                    <div
                      style={{ color: "rgb(1, 107, 188)", textAlign: "center" }}
                    >
                      {" "}
