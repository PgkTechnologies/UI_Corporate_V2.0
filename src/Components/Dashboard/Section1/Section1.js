...
} from "../../../Store/Actions/SagaActions/AnalyticsSagaActions";
...
const Section1 = (props) => {
  ...
  const [TwentytwoPowerBi, setTwentytwoPowerBi] = useState("");
  ...
  const loadPowerBi = (chart) => {
    if (chart === 3) {
      ...
      chart === 22 &&
      reportInfomation.length > 0 &&
      profileInfo?.stakeholderID
    ) {
      setTwentytwoPowerBi(
        <PowerBIEmbed
          embedConfig={
            getReportID("corporate_wise_applied_students").length > 0
              ? {
                  type: "report", // Supported types: report, dashboard, tile, visual and qna
                  id: getReportID("corporate_wise_applied_students"),
                  embedUrl: `https://app.powerbi.com/reportEmbed?reportId=${getReportID(
                    "corporate_wise_applied_students"
                  )}`,
                  accessToken: accessToken, // Keep as empty string, null or undefined
                  settings: {
                    filterPaneEnabled: false,
                  ...
                  filters: [
                    {
                      ...
                      target: {
                        $schema: "http://powerbi.com/product/schema#column",
                        table:
                          "C2Hire_Reporting CorporateWiseAppliedStudentList",
                        column: "Corporate_Stakeholder_ID",
                      },
                      // @ts-ignore
                      operator: "Is",
      ...
      <div className="row">
        <div className="col-md-12">
          <Accordion className="mt-12">
            ...
            <AccordionDetails>
              <div className="w-100">
                <div className="col">{TwentytwoPowerBi}</div>
              </div>
            </AccordionDetails>
          </Accordion>
        </div>
